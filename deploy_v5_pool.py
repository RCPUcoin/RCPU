...
function constructCoinbase(template, minerAddress, extraNonce1, extraNonce2) {
    ...
    const scriptSigContent = Buffer.from(heightBytes).toString('hex') + extranonce;
...
async function submitBlockToNode(job, nonce, ntime, minerAddress, extraNonce1, extraNonce2) {
    try {
        ...
        const coinbaseTx = constructCoinbase(job.template, addr, xnonce1, xnonce2);
        const coinbaseTxId = calculateTxId(coinbaseTx);
        
        log("Coinbase txid: " + coinbaseTxId);
        log("Coinbase scriptSig length: " + coinbaseTx.substring(64, 68));
        ...
        const coinbaseWitness = '01' + '20' + '00'.repeat(32);
        
        let txHex = coinbaseTx;
        if (job.template.transactions) {
            for (const tx of job.template.transactions) {
                txHex += tx.data;
            }
        ...
        const blockHex = blockHeader + txCountHex + witnessMarker + witnessFlag + coinbaseWitness + txHex;
...
print("=== 部署V5矿池代码 ===")
print("修复的致命问题:")
print("  1. coinbase scriptSig = push(height) + extranonce (符合BIP34规范)")
print("  2. 区块提交使用SegWit序列化格式 (marker+flag+witness_stack)")
print("  3. merkle根使用不含witness的txid计算")
print("  4. xnonce1设为空字符串，xnonce2_size=4")
print("  5. coinb1/coinb2正确构造，extranonce插在scriptSig末尾")
print("  6. 使用GBT的transactions列表计算完整merkle根")
print("  7. 硬编码scriptPubKey避免bech32解码错误")
...
try:
    ...
    print("\n强制停止所有node进程...")
    stdin, stdout, stderr = ssh.exec_command('pkill -9 node 2>/dev/null; sleep 3')
    stdout.channel.recv_exit_status()
    print("进程已停止")
    
    print("\n上传代码到服务器...")
    stdin, stdout, stderr = ssh.exec_command('cat > /root/stratum-proxy-pool.js << "ENDOFSCRIPT"\n' + pool_code + '\nENDOFSCRIPT')
    stdout.channel.recv_exit_status()
    print("代码上传完成")
    ...
    print("\n" + "="*60)
    print("V5矿池部署完成！")
    print("关键修复: 符合节点期望的coinbase结构和SegWit序列化")
    print("="*60)
