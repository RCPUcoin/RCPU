...
def main():
    ...
    try:
        ...
        sftp.get('/root/.rcpu/wallet_backup.tar.gz', '/tmp/wallet_backup.tar.gz')
        sftp.close()
        print("已下载到本地临时目录")
        ...
        sftp.put('/tmp/wallet_backup.tar.gz', '/root/.rcpu/wallet_backup.tar.gz')
        sftp.close()
        print("已上传到目标服务器")
        ...
        os.remove('/tmp/wallet_backup.tar.gz')
