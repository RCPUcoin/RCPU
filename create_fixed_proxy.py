...
async function getBlockTemplate() {
    try {
        const response = await makeRpcRequest('getblocktemplate', [{}]);
        return response;
    } catch (e) {
...
(async () => {
    ...
    if (template) {
        ...
        log(`Initial job: height=${currentJob.height}, diff=${currentJob.networkDifficulty.toFixed(2)}`);
...
setInterval(async () => {
    ...
    if (template) {
        ...
        log(`New job broadcast: ${job.job_id}, height=${job.height}, diff=${job.networkDifficulty.toFixed(2)}`);
    }
}, 60000);
'''
