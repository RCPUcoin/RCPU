...
On unix systems, the `--enable-multiprocess` build option can be passed to `./configure` to build new `bitcoin-node`, `rcpu-wallet`, and `bitcoin-gui` executables alongside existing `rcpud` and `rcpu-qt` executables.
