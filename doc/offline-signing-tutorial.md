...
```sh
[offline]$ ./src/rcpu-cli -signet -named createwallet \
...
```sh
[offline]$ ./src/rcpu-cli -signet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./src/rcpu-cli -signet -named createwallet \
...
```sh
[online]$ ./src/rcpu-cli -signet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./src/rcpu-cli -signet -rpcwallet="watch_only_wallet" getnewaddress
...
```sh
[online]$ ./src/rcpu-cli -signet -rpcwallet="watch_only_wallet" listunspent
...
```sh
[online]$ ./src/rcpu-cli -signet -rpcwallet="watch_only_wallet" send \
...
```sh
[offline]$ ./src/rcpu-cli -signet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./src/rcpu-cli -signet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./src/rcpu-cli -signet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./src/rcpu-cli -signet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./src/rcpu-cli -signet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./src/rcpu-cli -signet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./src/rcpu-cli -signet -rpcwallet="watch_only_wallet" listtransactions
